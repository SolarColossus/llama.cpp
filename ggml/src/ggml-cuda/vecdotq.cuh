...
static __device__ __forceinline__ float vec_dot_iq3_ks_q8_1(
    ...
    const block_iq3_ks * bq3 = (const block_iq3_ks *) vbq + kbx;

    int iqs4 = iqs/4;
    const int ib128 = iqs4/4;  // 0 or 1. 0 works on quants 0...127, 1 on quants 128...255
    const int il8   = iqs4%4;  // 0...3
    ...
    float result = d * (__low2float(bq8_1[4*ib128+0].ds) * (a8[0] + ((extra << 4) & 0x10)) * sumi[0] +
